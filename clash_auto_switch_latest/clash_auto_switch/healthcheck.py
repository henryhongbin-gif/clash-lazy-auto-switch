...
def proxy_score(
    ...
    for idx in range(max(1, samples)):
        with concurrent.futures.ThreadPoolExecutor(
            ...
            raw_delays: list[int | None] = []
            for future in futures:
                try:
                    raw_delays.append(future.result())
                except Exception:
                    raw_delays.append(None)

            if all(delay is None for delay in raw_delays):
                return None

            round_delays = [
                delay if delay is not None else TIMEOUT_PENALTY_MS
